...
/// before decoding, exactly as [`read_series_rescaled`] with `strip_phi = true`. `mode` (#300) is
/// forwarded so crypto-shred composes cleanly with `GlobalInt16`.
pub fn read_series_crypto_shred(
    ...
    let identity = capture_identity(&obj);
    Ok((read_series_inner(paths, true, mode)?, identity))
}
