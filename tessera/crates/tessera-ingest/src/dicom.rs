...
fn read_series_inner(
    ...
    let slopes_uniform = slices
        .iter()
        .all(|(_, _, s)| s.rescale_slope == first.rescale_slope);
    ...
    match mode {
        ...
        RescaleMode::GlobalInt16 => {
            ...
            let gmax = slices
                .iter()
                .flat_map(|(_, _, s)| s.voxels.iter().map(|&v| (v as f64 * s.rescale_slope).abs()))
                .fold(0.0f64, f64::max);
            ...
            let per_slice_slopes: Vec<f64> =
                slices.iter().map(|(_, _, s)| s.rescale_slope).collect();
            let mut voxels = Vec::with_capacity(slices.len() * first.voxels.len());
